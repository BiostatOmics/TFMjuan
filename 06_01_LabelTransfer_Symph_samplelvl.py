...
sp.pp.harmony_integrate(ref, key="sample_id", verbose=True, random_seed=2)
...
sp.tl.map_embedding(query, ref, key="sample_id")
...
log_message("7.1 Obteniendo vecinos de query integrado (a nivel de symphony)...")
sc.pp.neighbors(query, use_rep="X_pca_harmony_symphony_R",
                n_neighbors=30, n_pcs=50, metric="manhattan", key_added="symph")
...
log_message("7.2 Obteniendo clústers de query integrado (a nivel de symphony)...")
sc.tl.leiden(query, resolution=0.25, flavor="leidenalg",
             key_added="leiden_0_25_symp", neighbors_key="symph")

log_message("7.3 Obtención de scores (a nivel de harmony)...")
sp.tl.per_cluster_confidence(query, ref,
                             cluster_key="leiden_0_25_harm",
                             )
query.obs["harmony_per_cluster_dist"] = query.obs["symphony_per_cluster_dist"]

log_message("7.3 Obtención de scores (a nivel de symphony)...")
sp.tl.per_cluster_confidence(query, ref,
                             cluster_key="leiden_0_25_symp",
                             )
...
categoric_to_plot = ["supercluster_term",
                     ...
                     "leiden_0_25_symp",  # este es el interesante
                     "set",
                     "case_control",
                     "brain_region",
                     "Phase",
